...
//#define D    (uint16_t)587
//#define E    (uint16_t)659
#define Fs   (uint16_t)740
//#define G    (uint16_t)784
//#define A    (uint16_t)880
//#define B    (uint16_t)988
...
void SEGTIM_OC_GPIO_Config(void);
...
int main(void)
{
  SystemClock_Config();
	...
	while(1)
	{
		...
		adc_data = ADC1->DR;
		if(adc_data<0x0100)
			{ij[3] = 0 ;ij[2] = 0 ;ij[1] = 0 ;ij[0] = 0;}
		else if(adc_data<0x0200)
			{ij[3] = 0 ;ij[2] = 1 ;ij[1] = 0 ;ij[0] = 0;}
		else if(adc_data<0x0400)
			{ij[3] = 0 ;ij[2] = 2 ;ij[1] = 0 ;ij[0] = 0;}
		else if(adc_data<0x0600)
			{ij[3] = 0 ;ij[2] = 3 ;ij[1] = 0 ;ij[0] = 0;}
		else if(adc_data<0x0700)
			{ij[3] = 0 ;ij[2] = 4 ;ij[1] = 0 ;ij[0] = 0;}
		else if(adc_data<0x0800)
			{ij[3] = 0 ;ij[2] = 5 ;ij[1] = 0 ;ij[0] = 0;}
		else if(adc_data<0x0FFF)
			{ij[3] = 1 ;ij[2] = 0 ;ij[1] = 0 ;ij[0] = 0;}
		if(adc_data<0x0800 && music==0){
...
//				LL_TIM_SetAutoReload(TIM4, MUTE); //Change ARR of Timer PWM
		...
		else if(adc_data<0x0800 && music==1){
			for(int i=0;i<sizeof(tetris)/4;)
			{
				if(LL_TIM_IsActiveFlag_UPDATE(TIM2) == SET )
				{
					LL_TIM_SetAutoReload(TIM4,ARR_CALCULATE(tetris[i]));
					LL_TIM_OC_SetCompareCH1(TIM4,LL_TIM_GetAutoReload(TIM4) / 2);
					//TIM_OC_Config(ARR_CALCULATE(sheetnote[i]));
					if(tetris[i]==MUTE){LL_TIM_SetAutoReload(TIM2,MUTE_ARR);}
					else if(tetris[i+1]==MUTE){LL_TIM_SetAutoReload(TIM2,LAST_EIGHT_NOTE_ARR);}
					else{LL_TIM_SetAutoReload(TIM2,EIGHT_NOTE_ARR);}
					++i;
					LL_TIM_ClearFlag_UPDATE(TIM2);
//				LL_TIM_SetAutoReload(TIM4, MUTE); //Change ARR of Timer PWM
			...
			for(int i=0;i<sizeof(Pirate)/4;)
			{
				if(LL_TIM_IsActiveFlag_UPDATE(TIM2) == SET )
				{
					LL_TIM_SetAutoReload(TIM4,ARR_CALCULATE(Pirate[i]));
					LL_TIM_OC_SetCompareCH1(TIM4,LL_TIM_GetAutoReload(TIM4) / 2);
					//TIM_OC_Config(ARR_CALCULATE(sheetnote[i]));
					if(Pirate[i]==MUTE){LL_TIM_SetAutoReload(TIM2,MUTE_ARR);}
					else if(Pirate[i+1]==MUTE){LL_TIM_SetAutoReload(TIM2,LAST_EIGHT_NOTE_ARR);}
					else{LL_TIM_SetAutoReload(TIM2,EIGHT_NOTE_ARR);}
					++i;
					LL_TIM_ClearFlag_UPDATE(TIM2);
//				LL_TIM_SetAutoReload(TIM4, MUTE); //Change ARR of Timer PWM
